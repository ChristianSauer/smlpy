...
def get_test_files():
    ...
    files = [
        x for x in data.iterdir() if x.suffix == ".hex" and "with_error" not in x.name
    ]
    files = sorted(files)
...
def test_own_data():
    ...
    assert isinstance(
        reader.sml_file.data[2].message_body, sml_reader.SmlPublicCloseRes
    )
...
def test_can_dump():
    ...
    assert (
        data["data"][1]["message_body"]["val_list"][4]["scaled_value"]
        == 8571286.200000001
    )
